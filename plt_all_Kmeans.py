...
plt.plot(gndvi_ndvi_monthly_stats['Year_Month'].dt.to_timestamp(), gndvi_ndvi_monthly_stats['mean_value_gndvi'],
         label='GNDVI Mean', color='red', marker=None, linewidth=0.8)
...
plt.plot(gndvi_ndvi_monthly_stats['Year_Month'].dt.to_timestamp(), gndvi_ndvi_monthly_stats['mean_value_ndvi'],
         label='NDVI Mean', color='#DE6E66', marker=None, linewidth=0.8)
...
plt.scatter(gndvi_ndvi_monthly_stats['Year_Month'].dt.to_timestamp(), gndvi_ndvi_monthly_stats['mean_value_ndvi'],
            c=gndvi_ndvi_monthly_stats['ndvi_cluster'], cmap='viridis', marker='o', s=10)
...
# Plot VV mean as a line plot
plt.plot(vv_monthly_stats['Year_Month'].dt.to_timestamp(), vv_monthly_stats['mean_value_vv'],
         label='VV Coherence Mean', color='blue', marker=None, linewidth=0.8)
# Define cluster colors


# Scatter VH with clusters
plt.scatter(vh_monthly_stats['Year_Month'].dt.to_timestamp(), vh_monthly_stats['mean_value_vh'],
            c=vh_monthly_stats['vh_cluster'], cmap='viridis', marker='o', s=10)
...
plt.ylim(0, 5.0)
plt.title('NDVI, GNDVI, VV, VH Variation of Wheat Over Time in 2023', fontsize=20)
plt.xlabel('Date', fontsize=16)
plt.ylabel('Index Value', fontsize=16)
plt.legend(loc='upper right', fontsize=14, frameon=False, bbox_to_anchor=(1, 0.5))
cbar = plt.colorbar(scatter)
cbar.set_label('Cluster', fontsize=16)
cbar.ax.tick_params(labelsize=14)
# Show plot
plt.tight_layout()
output_file_path = r'D:\BaiduNetdiskDownload\2023\wheat\kmeans_wheat_all_monthly_over_time.jpg'
